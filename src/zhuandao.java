    	    ...
    	    public void keyPressed(KeyEvent eee) {
    	     ...
    	     if (eee.getKeyCode() == KeyEvent.VK_ENTER)  
    	     {
    				...
    				try{
    					 ...
    					 int linenum = 0;
    		             linenum = frameMain.text.getLineOfOffset(frameMain.text.getText().length());
    					...
    					int position=frameMain.text.getLineStartOffset(hangshu);
    				    
    					JOptionPane.showMessageDialog(null, "系统已经为您自动转到了第"+hangshu+"行");
    				     frameMain.text.setCaretPosition(position-1);
    					setVisible(false);
    				 }
